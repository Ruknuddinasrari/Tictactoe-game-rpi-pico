...
void reset_board(char *current_player, uint *moves, char (*board)[COLS], bool *is_game_over) {
    ...
    *is_game_over = false;
    ...
    print_board(board);
    // Call the function "print_player_turn" with the current player as an argument
    print_player_turn(*current_player);
    // Call the function "multicore_fifo_push_blocking" with "EMPTY" as an argument
...
void update_board(const char current_player, const uint moves, char (*board)[COLS]) {
    // Calculate the row by calling "get_curr_row" function with the number of moves
    uint row = get_curr_row(moves);

    // Calculate the col by calling "get_curr_col" function with the number of moves
    uint col = get_curr_col(moves);

    // Print the current player and the row and col where the player's input is being entered
...
void flash_winner_led() {
  // Declare a variable "winner" of type uint32_t and initialize it to the value of "EMPTY"
  uint32_t winner = (uint32_t) EMPTY;
  
  // Declare a variable "led_pin" of type uint and initialize it to the value of "ONBOARD_LED"
  uint led_pin = ONBOARD_LED;

  // Start an infinite loop
  while (true) {
    if (multicore_fifo_rvalid()) {
      // Pop a value from the "multicore_fifo" and store it in the variable "winner"
    ...
    // If the value of "winner" is neither 'X' nor 'O', set the value of "led_pin" to "ONBOARD_LED"
    else {
      led_pin = ONBOARD_LED;
    }

    // Set the value of "led_pin" to "HIGH"
